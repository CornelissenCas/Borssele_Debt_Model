...
```
pip install numpy numpy-financial pandas matplotlib ipywidgets
```

Place `NL_wholesale_electricity_prices.csv` in a `data/` folder alongside the notebook, then restart the kernel and run all cells in order. The sensitivity dashboard in the final section includes an interactive, slider-driven version that only renders live inside a running Jupyter session.
